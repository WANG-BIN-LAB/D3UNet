...
def calc(seg, idx):
    ...
    roi = {'min_x': min_y, 'min_y': min_x, 'min_z': min_z,
           'max_x': max_y, 'max_y': max_x, 'max_z': max_z,
...
def get_roi_from_gt(data_path, roi_file):
    ...
    for case in tqdm(cases, ascii=True, dynamic_ncols=True):
        img_file = case / (str(case.name) + '_teeth.nii.gz')


        print(img_file)
        assert img_file.exists()
        img = nib.load(str(img_file)).get_data()
        total_x, total_y, total_z = img.shape #640 640 528
        vol = {'total_x': total_x, 'total_y': total_y, 'total_z': total_z}
        case_data = {'vol': vol}
        
        seg_file = case / ('prediction_' + str(case.name) + '.nii.gz')
        print(seg_file)
        if seg_file.exists():
            seg = nib.load(str(seg_file)).get_data()
            teeth = calc(seg, idx=1)
            root = calc(seg, idx=2)
            case_data.update({'teeth': teeth, 'root': root})
        
        rois[f'{case.name}'] = case_data
...
def get_roi_from_resunet(batch_size, num_gpu, img_size, data_path, resume, roi_file, vis_intvl, num_workers):
    ...
    transform = TeethTransform(output_size=img_size, roi_error_range=15, use_roi=False)
    
    dataset = Teeth(data_path, stack_num=3, spec_classes=[0, 1, 1], img_size=img_size,
...
#            print(f'Processing sample: {idx[0]}')
            ...
            while case < len(case_slice_indices) - 1 and idx[-1] >= case_slice_indices[case + 1] - 1:
                ...
                teeth = calc(vol, idx=1)
                case_roi = {'teeth': teeth}
                case_id = dataset.case_idx_to_case_id(case, 'test')
                print(str(case_id))
                rois[f'{case_id}'].update(case_roi)
                with open(roi_file, 'w') as f:
                    json.dump(rois, f, indent=4, separators=(',', ': '))
...
@click.command()
@click.option('--org_data', 'org_data_path', help='teeth data path',
              type=click.Path(exists=True, dir_okay=True, resolve_path=True),
              default='/home/wr/D3UNet/data_good_xf2', show_default=True)
@click.option('--data', 'data_path', help='Path of teeth data after conversion',
              type=click.Path(exists=True, dir_okay=True, resolve_path=True),
              default='/home/wr/D3UNet/data_good', show_default=True)
@click.option('-o', '--output', 'roi_file', help='Output roi file path',
              type=click.Path(file_okay=True, resolve_path=True), default='/home/wr/D3UNet/roi.json', show_default=True)
...
@click.option('-r', '--resume', help='Resume model',
              type=click.Path(exists=True, file_okay=True, resolve_path=True), default='/home/wr/D3UNet/runsTeeth/1711604682.0538008/checkpoint/best.pth',required=True)
