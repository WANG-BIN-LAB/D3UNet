...
from dataset.teethTransform import to_numpy


class TeethEdge(data.Dataset):
    def __init__(self, root, stack_num=1, spec_classes=None, img_size=(512, 512),
                 train_case_ids_file='train.txt', valid_case_ids_file='val.txt', test_case_ids_file='test2.txt',
    ...
    def _get_data(self, train_case_ids_file, valid_case_ids_file, test_case_ids_file):
        ...
        self._valid_test_indices = self._indices[len(train_imgs):len(train_imgs) + len(valid_imgs) + len(test_imgs)]
    ...
    def _read_npy(self, root, cases, is_test=False):
        ...
        for case in cases:
            ...
            imaging_dir = case_root / 'teeth_img'
            ...
            if not is_test:
                ...
                case_labels = case_labels[min_x: max_x]
                labels += case_labels
    ...
    def get_classes_name(self, spec=True):
        classes_name = np.array(['background', 'teeth', 'root'])
    ...
    def vis_transform(self, data):
        ...
        # edge loss cal
        if 'teeth_edge' in data.keys() and data['teeth_edge'] is not None and data['teeth_edge'].shape[-1] != 0:
            teeth_edges = data['teeth_edge']
            if type(teeth_edges).__module__ != np.__name__:
                teeth_edges = teeth_edges.cpu().detach().numpy()
            teeth_edges = cmap[teeth_edges]
            teeth_edges = teeth_edges.transpose((0, 3, 1, 2))
            teeth_edges = teeth_edges / 255
            data['teeth_edge'] = teeth_edges

        if 'root_edge' in data.keys() and data['root_edge'] is not None and data['root_edge'].shape[-1] != 0:
            root_edges = data['root_edge']
            if type(root_edges).__module__ != np.__name__:
                root_edges = root_edges.cpu().detach().numpy()
            root_edges = cmap[root_edges]
            root_edges = root_edges.transpose((0, 3, 1, 2))
            root_edges = root_edges / 255
            data['root_edge'] = root_edges
    ...
    def _default_transform(self, data):
        ...
        if teeth_edge is not None:
            teeth_edge = teeth_edge.astype(np.int64)

            if self._spec_classes != [0, 1, 2]:
                idx = list(range(len(self.get_classes_name(spec=False))))
                masks = [np.where(teeth_edge == i) for i in idx]
                spec_class_idx = []
                for i in self._spec_classes:
                    if i not in spec_class_idx:
                        spec_class_idx.append(i)
                ...
                for mask, spec_class in zip(masks, self._spec_classes):
                    teeth_edge[mask] = spec_class_idx.index(spec_class)

            teeth_edge = torch.from_numpy(teeth_edge)
            data['teeth_edge'] = teeth_edge

        else:
            data['teeth_edge'] = torch.Tensor()


        if root_edge is not None:
            root_edge = root_edge.astype(np.int64)
            if self._spec_classes != [0, 1, 2]:
                idx = list(range(len(self.get_classes_name(spec=False))))
                masks = [np.where(root_edge == i) for i in idx]
                spec_class_idx = []
                for i in self._spec_classes:
                    if i not in spec_class_idx:
                        spec_class_idx.append(i)
                ...
                for mask, spec_class in zip(masks, self._spec_classes):
                    root_edge[mask] = spec_class_idx.index(spec_class)

            root_edge = torch.from_numpy(root_edge)
            data['root_edge'] = root_edge

        else:
            data['root_edge'] = torch.Tensor()
    ...
    def _resize(self, data):
        ...
        data = aug(image=img, masks=[label, teeth_edge, root_edge])

        img, label, teeth_edge, root_edge = data['image'], data['masks'][0], data['masks'][1], data['masks'][2]
    ...
    def get_stack_img(self, idx):
        ...
        if idx in self._test_indices:
            ...
            root_area = np.where(label > 1, label, 0)
            teeth_area = label - root_area 
            boundary_teeth = find_boundaries(teeth_area, mode='inner').astype(np.int16)
            boundary_root = find_boundaries(root_area, mode='inner').astype(np.int16)


        roi = self.get_roi(case_idx, type='all')['teeth'] if self._use_roi else {}
        data = {'image': img, 'label': label, 'teeth_edge': boundary_teeth, 'root_edge': boundary_root, 'index': idx, 'roi': roi}
...
@click.command()
@click.option('--data', 'data_path', help='teeth data path',
...
def main(data_path):
    ...
    transform.eval()
    dataset = TeethEdge(root, stack_num=3, spec_classes=[0, 1, 2], img_size=(512, 512),
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        imshow(title='TeethKI', imgs=(imgs[0][1], labels[0]))
