...
class Teeth(data.Dataset):
    def __init__(self, root, stack_num=1, spec_classes=None, img_size=(512, 512),
                 ...
                 use_roi=True, roi_file=None, roi_error_range=0,
    ...
    def _get_data(self, train_case_ids_file, valid_case_ids_file, test_case_ids_file):
        ...
        self._labels = train_labels + valid_labels + test_labels
        # self._labels_e = train_labels_e

        self._indices = list(range(len(self._imgs)))
        # self._indices_e = list(range(len(self._labels_e)))

        self._train_indices = self._indices[:len(train_imgs)]

        # self._train_e_indices = self._indices_e[:len(train_labels_e)]
...
def main(data_path):
    ...
    transform.eval()
    dataset = Teeth(root, stack_num=3, spec_classes=[0, 1, 2], img_size=(512, 512),
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        imshow(title='Teeth', imgs=(imgs[0][1], labels[0]))
