...
@click.command()
@click.option('-e', '--epoch', 'epoch_num', help='Number of training epoch', type=int, default=50, show_default=True)
...
@click.option('-d', '--data', 'data_path', help='Path of teeth/root data after conversion',
...
@click.option('--log', 'log_path', help='Checkpoint and log file save path',
              ...
              default='/home/wr/MD2UNet/runsTeeth', show_default=True)
...
def main(epoch_num, batch_size, lr, num_gpu, img_size, data_path, log_path,
    ...
    cp_path = log_path / time_str / 'checkpoint'

    if not resume and cp_path.exists() and len(list(log_path.glob('*'))) > 0:
